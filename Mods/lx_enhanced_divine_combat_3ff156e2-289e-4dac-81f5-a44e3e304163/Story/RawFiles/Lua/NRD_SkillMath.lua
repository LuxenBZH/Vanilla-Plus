------ Credits to Norbyte for this awesome script ------

local DamageTypeToDeathTypeMap = {
    Physical = "Physical",
    Piercing = "Piercing",
    Fire = "Incinerate",
    Air = "Electrocution",
...
function DamageTypeToDeathType(damageType)
    ...
    if deathType ~= nil then
        ...
        -- FIXME or "Sulfur" /editor uses this value/?
        return "Sentinel"
    end
end
...
function GetPrimaryAttributeAmount(skill, attacker)
    -- FIXME - partial implementation - UseWeaponDamage=1 part unimplemented
    if skill.UseWeaponDamage == "Yes" then
        error("UseWeaponDamage=1 not supported yet")
    end

    local ability = skill.Ability
    Ext.Print("GPA " .. tostring(attacker))
    if ability == "Warrior" or ability == "Polymorph" then -- 1, 11
        return attacker.Strength
    elseif ability == "Ranger" or ability == "Rogue" then -- 2, 3
        return attacker.Finesse
    else
...
function GetSkillAttributeDamageScale(skill, attacker)
    if attacker == nil or skill.UseWeaponDamage == "Yes" or skill.Ability == 0 then
        ...
        Ext.Print("Call SDFPA " .. type(primaryAttr))
        Ext.Print(attacker)
...
function ApplyDamageBoosts(character, damageList)
    for i, damage in pairs(damageList:ToTable()) do
        ...
        if boost > 0.0 then
            Ext.Print("DamageBoost for ", damage.DamageType, boost)
            damageList:Add(damage.DamageType, damage.Amount * boost)
        end
    end
end
...
function ComputeBaseWeaponDamage(weapon)
    ...
    for i, stat in pairs(stats) do
        if stat.StatsType == "Weapon" and stat.DamageType ~= "None" then
            Ext.Print("Process layer ", i, stat.StatsType, stat.DamageType)
            ...
            Ext.Print("Weapon base dmg add: ", dmgType, minDamage, maxDamage)
    ...
    Ext.Print("Final weapon base boost is ", damageBoost)
...
function CalculateWeaponScaledDamage(character, weapon, damageList, noRandomization)
    ...
    Ext.Print("abilityBoosts0 ", abilityBoosts)
    abilityBoosts = math.max(abilityBoosts + 100.0, 0.0) / 100.0
    Ext.Print("abilityBoosts ", abilityBoosts)
    ...
    for damageType, damage in pairs(damages) do
        ...
        Ext.Print("add ", damageType, finalAmount)
...
function CustomGetSkillDamage(skill, attacker, isFromItem, stealthed, attackerPos, targetPos, level, noRandomization)
    Ext.Print(skill, attacker, isFromItem, stealthed, attackerPos, targetPos, level, noRandomization)
    ...
    Ext.Print("damageMultiplier is ", damageMultiplier)
    if damageMultiplier <= 0 then
        return
    end

    Ext.Print("UseWeaponDamage is ", skill.UseWeaponDamage)
    if skill.UseWeaponDamage == "Yes" then
        ...
        Ext.Print("Weapon dmg calc: dmgType ", damageType, " weapons ", weapon, offHand)
        if weapon ~= nil then
        ...
        Ext.Print("Base damage ", baseDamage)
        ...
        Ext.Print("skill.Damage ", skill.Damage)
        ...
        Ext.Print("Boosts ", randomMultiplier, attrDamageScale, damageMultipliers, damageBoost)
        ...
        Ext.Print("Final damage ", finalDamage)
    ...
    Ext.Print("--- FINAL DAMAGE LIST ---")
    for i, damage in pairs(damageList:ToTable()) do
        Ext.Print("DamageBoost for ", damage.DamageType, damage.Amount)
    end
    Ext.Print("--- DONE ---")
...
--Ext.RegisterListener("GetSkillDamage", CustomGetSkillDamage)



-- FIXME - special RNG!
-- stats.CritChance.Roll()
...
function ApplyHitResistances(character, damageList)
    for i,damage in pairs(damageList:ToTable()) do
        ...
        Ext.Print(damage.DamageType .. " -> " .. resistance)
...
function ApplyDamageCharacterBonuses(character, attacker, damageList)
    ...
    Ext.Print("After ApplyHitResistances")
    for i,damage in pairs(damageList:ToTable()) do
...
function DoHit(hit, damageList, statusBonusDmgTypes, hitType, target, attacker)
    ...
    Ext.Print("DamageMultiplier", hit.DamageMultiplier)
    ...
    Ext.Print("DoHit final: flags ", hit.EffectFlags, ", damage ", hit.TotalDamageDone, ", absorption ", hit.ArmorAbsorption, ", lifesteal ", hit.LifeSteal)
    for i,damage in pairs(hit.DamageList:ToTable()) do
        Ext.Print("Damage: ", damage.DamageType, damage.Amount)
    end
    Ext.Print(" --- END --- ")
end
...
function HitInternal(target, attacker, weapon, damageList, hitType, rollForDamage, forceReduceDurability, hit, alwaysBackstab, highGroundFlag, criticalRoll)
    Ext.Print(" ===== HIT INTERNAL ===== ")
    Ext.Print(target, attacker, weapon, damageList, hitType, rollForDamage, forceReduceDurability, hit, alwaysBackstab, highGroundFlag, criticalRoll)
    ...
    local backstabbed = 0
    if alwaysBackstab or (weapon.WeaponType == "Knife" and CanBackstab(target, attacker)) then
    ...
    if rollForDamage ~= 0 then
        local hitChance = CalculateHitChance(target, attacker)
        local hitRoll = math.random(0, 99)
		Ext.Print("HitChance:", hitChance, "HitRoll:",hitRoll, target.TALENT_RangerLoreEvasionBonus)
        if hitRoll >= hitChance then
            ...
            if backstabbed == false and blockChance > 0 and math.random(0, 99) <= blockChance then
    ...
    if weapon.Name ~= "DefaultWeapon" and hitType ~= "Magic" and forceReduceDurability and (hit.EffectFlags & (HitFlag.Missed|HitFlag.Dodged)) == 0 then
...
--Ext.RegisterListener("ComputeCharacterHit", HitInternal)
