...
HitManager:RegisterHitListener("DGM_Hit", "AfterDamageScaling", "VP_AbsorbShields", function(hit, instigator, target, flags)
	...
	if hit.SkillId ~= "" then
		...
		if stat.VP_ConsecutiveDamageReductionPercent ~= 0 or string.starts(hit.SkillId, "Target_DualWieldingAttack") then
			if stat.VP_ConsecutiveDamageReductionHitAmount > 0 or string.starts(hit.SkillId, "Target_DualWieldingAttack") then
